...
from kyselyt.constants import VALSSI_VERSION, UI_LOG_CHAR_LIMIT
from kyselyt.decorators import custom_ratelimit
from kyselyt.enums.error_messages import ErrorMessages
from kyselyt.models import Kysely, Kyselykerta, Vastaus, VastausSend, TempVastaus, Kysymysryhma
...
from kyselyt.utils import (
    validate_language_code, check_celery_worker_running, get_ci_pipeline_number, validate_log_timestamp,
    validate_language_code_by_kysymysryhma,
)
...
class VastaaViewSet(GenericViewSet):
    ...
    def create(self, request):
        ...
        kysymysryhma = Kysymysryhma.objects.filter(
            kyselykysymysryhma__kyselyid=serializer.vastaajatunnus.kyselykertaid.kyselyid
        ).first()
        if kysymysryhma is None:
            raise NotFound([ErrorMessages.AR007.value])  # HTTP_404_NOT_FOUND

        validate_language_code_by_kysymysryhma(language, kysymysryhma)

        # Create Vastaaja after validations
...
class VastaajaUILoggingViewSet(APIView):
    def post(self, request):
        try:
            log_timestamp = int(request.data["time"])
            errlog = str(request.data["errlog"])
            if validate_log_timestamp(log_timestamp) and errlog:
                logger.warning(f"[VASTAAJAUIERRLOG] {errlog[:UI_LOG_CHAR_LIMIT]}")
        except Exception:
            pass
...
class HealthDbViewSet(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            return Response("Vastauspalvelu-db connection error.", status=status.HTTP_503_SERVICE_UNAVAILABLE)
