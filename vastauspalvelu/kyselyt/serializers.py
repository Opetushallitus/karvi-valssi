...
from kyselyt.models import (
    ...
    FailedTaustatiedot, KysymysJatkokysymys)
...
class JatkoKysymysSerializer(serializers.ModelSerializer):
    jatkokysymys_vastaus = SerializerMethodField()
    jatkokysymys_kysymysid = SerializerMethodField()
    ...
    class Meta:
        ...
        fields = ["kysymysid",
                  ...
                  "eos_vastaus_sallittu",
                  "jatkokysymys",
                  "jatkokysymys_vastaus",
                  "jatkokysymys_kysymysid",
                  "rajoite",
                  "selite_fi",
                  "selite_sv",
                  "selite_en",
    ...
    def get_jatkokysymys_vastaus(self, instance):
        jatkokysymys_obj = KysymysJatkokysymys.objects.filter(jatkokysymysid=instance).first()
        if not jatkokysymys_obj:
            return None
        return jatkokysymys_obj.vastaus

    def get_jatkokysymys_kysymysid(self, instance):
        jatkokysymys_obj = KysymysJatkokysymys.objects.filter(jatkokysymysid=instance).first()
        if not jatkokysymys_obj:
            return None
        return jatkokysymys_obj.kysymysid.pk
...
class KysymysSerializer(serializers.ModelSerializer):
    ...
    jatkokysymykset = SerializerMethodField()
    ...
    def get_matriisikysymykset(self, instance):
        if instance.vastaustyyppi != "matrix_root":
            return []
        matriisikysymykset = Kysymys.objects.filter(
            matriisi_kysymysid=instance.kysymysid,
            matriisi_jarjestys__gt=0).order_by("matriisi_jarjestys")
        return MatriisiKysymysSerializer(matriisikysymykset, many=True, read_only=True).data

    def get_jatkokysymykset(self, instance):
        jatkokysymykset = Kysymys.objects.filter(kysymysjatkokysymys_jatkokysymysid__kysymysid=instance)
        if not jatkokysymykset.exists():
            return {}
        jatkokysymykset_list = JatkoKysymysSerializer(jatkokysymykset, many=True, read_only=True).data
        return {jatkokysymys_item["jatkokysymys_vastaus"]: jatkokysymys_item
                for jatkokysymys_item in jatkokysymykset_list}
...
class KysymysryhmaSerializer(serializers.ModelSerializer):
    ...
    def get_kysymykset(self, instance):
        ...
        kysymykset = (
            ...
            instance.kysymykset.filter(matriisi_jarjestys=None)).exclude(jatkokysymys=True).order_by("jarjestys")
...
class VastaajaPostSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if self.vastaajatunnus.kohteiden_lkm == 1 and self.vastaajatunnus.kyselykertaid.kyselyid.oppilaitos:
            tyontekija_data, is_tyontekija_not_found = get_tyontekija_data(self.vastaajatunnus.tunnus)
            if tyontekija_data is not None:
                vastaaja_data.update({k: tyontekija_data[k] for k in ("tehtavanimikkeet", "tutkinnot")})
            elif is_tyontekija_not_found:
                logger.warning(f"VastaajaPostSerializer: tyontekija not found. ({tunnus})")
            else:
                logger.error(
                    f"VastaajaPostSerializer failed to get tyontekija_data from Virkailijapalvelu. ({tunnus})")
                FailedTaustatiedot.objects.create(vastaajatunnus=tunnus)
