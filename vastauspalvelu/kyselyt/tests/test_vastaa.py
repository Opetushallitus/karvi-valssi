...
class VastaaApiTests(TestCase):
    ...
    def test_post_vastaa_language_missing_in_kysymysryhma(self):
        kysymysid = Kysymys.objects.get(kysymys_fi="testikysymys1_2").kysymysid
        data = {
        ...
        resp = self.client.post(VASTAUSPALVELU_API_URL + "/vastaa/?language=en", data, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue("DY003" in str(resp.json()))
