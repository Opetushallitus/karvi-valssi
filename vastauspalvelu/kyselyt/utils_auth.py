...
from kyselyt.constants import (
    VIRKAILIJAPALVELU_SETTINGS, SUCCESS_STATUSES, MAX_NO_OF_LOOPS, TYONTEKIJA_NOT_FOUND_ERROR_CODES,
...
def get_tyontekija_data(vastaajatunnus: str):
    for i in range(MAX_NO_OF_LOOPS):
        ...
        elif resp.status_code in TYONTEKIJA_NOT_FOUND_STATUSES and is_tyontekija_not_found(resp):
            return None, True
        else:
...
def is_tyontekija_not_found(response):
    try:
        response_json = response.json()
        for item in response_json:
            if item["error_code"] in TYONTEKIJA_NOT_FOUND_ERROR_CODES:
                return True
    except Exception:
        return False
    return False
