...
SELECT jatkokysymysid, kysymysid, vastaus FROM kysymys_jatkokysymys WHERE kysymysid = :kysymysid;


-- :name jatkokysymysliitos-olemassa! ? :1
SELECT EXISTS (SELECT 1 FROM kysymys_jatkokysymys
               WHERE kysymysid = :kysymysid
                 and jatkokysymysid = :jatkokysymysid
                 and vastaus = :vastaus);
...
-- :name poista-kysymyksen-jatkokysymysliitokset! :! :n
DELETE FROM kysymys_jatkokysymys WHERE kysymysid = :kysymysid;

-- :name poista-jatkokysymysliitos! :! :n
DELETE FROM kysymys_jatkokysymys
WHERE kysymysid = :kysymysid
  and jatkokysymysid = :jatkokysymysid
  and vastaus = :vastaus;
