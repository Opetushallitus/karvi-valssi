...
INSERT INTO koulutustoimija_ja_tutkinto (koulutustoimija, tutkinto, voimassa_alkupvm, voimassa_loppupvm, laaja_oppisopimuskoulutus)
    SELECT o.oid, :tutkintotunnus, to_date(:alkupvm, 'YYYY-MM-DD'), to_date(:loppupvm, 'YYYY-MM-DD'), :laaja_oppisopimuskoulutus
    FROM organisaatio o
    WHERE o.ytunnus = :ytunnus
...
SELECT DISTINCT k.*
FROM organisaatio k
JOIN koulutustoimija_ja_tutkinto ktt ON k.oid = ktt.koulutustoimija
...
SELECT * FROM tutkintotyyppi ORDER BY tutkintotyyppi ASC;

-- :name hae-kayttajan-tutkintotyypit :? :*
SELECT tt.* FROM organisaatio o
...
WHERE o.parent_oid = :koulutustoimija
