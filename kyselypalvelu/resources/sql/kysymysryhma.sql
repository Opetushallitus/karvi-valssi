...
SELECT k.*,
       kr.nimi_fi AS kysymysryhma_fi, kr.nimi_sv AS kysymysryhma_sv, kr.nimi_en AS kysymysryhma_en,
       kr.taustakysymykset AS taustakysymys,
       k.metatiedot AS kysymys_metatiedot,
...
--~(if (not (:yllapitaja? params)) "OR (kr.valtakunnallinen = TRUE AND kr.tila IN ('julkaistu', 'lukittu'))")
...
-- :name aseta-kysymysryhman-tila-julkaistu! :! :n
UPDATE kysymysryhma
SET tila = 'julkaistu', muutettu_kayttaja = :kayttaja,
    metatiedot = CASE
        WHEN (metatiedot->>'julkaisuaika') IS NULL
        THEN jsonb_set(metatiedot, '{julkaisuaika}', to_jsonb(to_char(CURRENT_DATE, 'YYYY-MM-DD')), true)
        ELSE metatiedot
    END
WHERE kysymysryhmaid = :kysymysryhmaid;
...
-- :name kayttoraja-ylitetty :? :*
with kyselykierros AS (
    select
        kr.kysymysryhmaid,
        k.voimassa_alkupvm--,
        --k.voimassa_loppupvm,
        --k.voimassa_alkupvm <= current_date AND (k.voimassa_loppupvm IS NULL OR k.voimassa_loppupvm >= current_date) as kaynnissa,
        --count(k.voimassa_alkupvm),
        --array_to_string(array_agg(k.kyselyid), ', ')
    from kysely k
         left join kysely_kysymysryhma kkr on k.kyselyid = kkr.kyselyid
         left join kysymysryhma kr on kkr.kysymysryhmaid = kr.kysymysryhmaid
    where k.koulutustoimija = :koulutustoimija
      and k.voimassa_alkupvm > (current_date - 30)
      and k.voimassa_loppupvm < current_date
    --~(if (:kysymysryhmaidt params) "AND kr.kysymysryhmaid IN (:v*:kysymysryhmaidt)")
    group by kr.kysymysryhmaid, k.voimassa_alkupvm
    order by k.voimassa_alkupvm desc
)
select
    count(kysymysryhmaid) > 2 as raja_ylitetty,
    kysymysryhmaid,
    (min(voimassa_alkupvm) + 30) as vanhin_pvm
from kyselykierros
