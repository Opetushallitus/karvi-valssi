(ns arvo.rest-api.kysymysryhma
  (:require
    ...
    [arvo.service.kysymysryhma :as kysymysryhma]
    ...
    [arvo.util :refer [add-index parse-int]]
    [clojure.tools.logging :as log]
...
(defn reitit []
  [[""
   ...
   ["/kayttoraja"
    {:conflicting true
     :get {:kayttooikeus :kysymysryhma
           :parameters {:query {(s/optional-key :kysymysryhmaid) [s/Int]}}
           :handler (fn [{{:keys [query]} :parameters}]
                        (let [id-params (:kysymysryhmaid query)
                              to-list (if (sequential? id-params) id-params [id-params])
                              id-list (->> to-list distinct (filter some?))]
                             (response-or-404 (kysymysryhma/kysymysryhma-kayttoraja id-list))))}}]
   ["/asteikot"
    {:conflicting true
     :get {:kayttooikeus :katselu
           :handler (fn [req]
