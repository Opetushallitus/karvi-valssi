     ...
     :post {:kayttooikeus :kysely
            ...
            :handler (fn [{{:keys [body]} :parameters}]
                       (let [kysely (assoc (paivita-arvot body
                             ...
                             parsitut-kyselykerrat (if (-> kysely :kyselykerrat not-empty)
                                                     (assoc kysely :kyselykerrat (map (fn [kk] (paivita-arvot kk [:voimassa_alkupvm :voimassa_loppupvm] parse-iso-date))
                                                                                      (:kyselykerrat kysely)))
                                                     kysely)
                             julkaistu-kysely (if (:tila parsitut-kyselykerrat)
                                                parsitut-kyselykerrat
                                                (assoc parsitut-kyselykerrat :tila "luonnos"))]
                         (validate-response arkisto/lisaa! [arkisto/validoi-lisaa! validate-util/validoi-oppilaitos] julkaistu-kysely)))}}]
   ["/massasyotto"
    {:conflicting true
     :post {:kayttooikeus :kysely
