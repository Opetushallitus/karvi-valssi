(ns arvo.arkisto.kysely
  (:require [arvo.arkisto.kyselykerta :as kyselykerta]
            [arvo.arkisto.kysymysryhma :as kysymysryhma]
...
(defn lisaa!
  ...
  ([tx kyselydata]
   (let [kysely (first (db/luo-kysely! tx (format-kysely kyselydata)))
        ...
        (assoc kyselydata
               :kyselykerrat (->> (:kyselykerrat kyselydata)
                                  (map (fn [kyselykerta] (merge kyselykerta (kyselykerta-arkisto/lisaa! tx kyselyid kyselykerta))))
                                  doall)
               :kyselyid kyselyid))))
...
(defn hae-kyselyn-kysymykset [kyselyid]
  (->> (db/hae-kyselyn-kysymysryhmat {:kyselyid kyselyid})
       (map kysymysryhma/hae-kysymysryhman-kysymykset)
       flatten))
