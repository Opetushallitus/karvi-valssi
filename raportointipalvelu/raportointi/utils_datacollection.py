...
def get_toimipaikka_extra_data(
    ...
    """
    toimipaikka_answer_percentage_gte_50 = []
    ...
    for kyselykerta in kyselykertas:
        ...
        if sent_count == 0:
            ...
            toimipaikka_answer_percentage_gte_50.append(
                {"nimi_fi": toimipaikka_name_fi, "nimi_sv": toimipaikka_name_sv, "answer_pct": answer_pct})

    toimipaikka_answer_percentage_lt_50_sorted = sorted(
        toimipaikka_answer_percentage_lt_50, key=lambda item: item["answer_pct"])

    toimipaikka_answer_percentage_gte_50_sorted = sorted(
        toimipaikka_answer_percentage_gte_50, key=lambda item: item["answer_pct"])

    return (toimipaikka_answer_percentage_gte_50_sorted,
            toimipaikka_answer_percentage_lt_50_sorted,
            toimipaikka_kysely_not_sent)
...
def get_data_dicts_by_kyselys(kyselys: QuerySet[Kysely]) -> (dict, dict, dict, dict, dict):
    ...
    for kysely in kyselys:
        kysely_dicts.append(dict(
            ...
            is_kunnallinen=kysely.koulutustoimija.metatiedot.get("is_kunnallinen", True)))
    ...
    for kysymysryhma in kysymysryhmas:
        kysymysryhma_dicts[kysymysryhma.pk] = dict(
            ...
            julkaisuaika=kysymysryhma.metatiedot.get("julkaisuaika", None),
    ...
    vastaajatunnuses = Vastaajatunnus.objects.filter(kyselykertaid__in=kyselykertaids) \
        .select_related("kyselykertaid", "kyselykertaid__kyselyid", "kyselykertaid__kyselyid__oppilaitos")
    vastaajatunnus_dicts = []
    oppilaitos_by_vastaajatunnus_dict = dict()
    for vastaajatunnus in vastaajatunnuses:
        oppilaitos_by_vastaajatunnus_dict[vastaajatunnus.tunnus] = \
            vastaajatunnus.kyselykertaid.kyselyid.oppilaitos.oid if (
                vastaajatunnus.kyselykertaid.kyselyid.oppilaitos) else None
    ...
    for vastaaja in vastaajas:
        oppilaitos = oppilaitos_by_vastaajatunnus_dict.get(vastaaja.vastaajatunnus, None)
