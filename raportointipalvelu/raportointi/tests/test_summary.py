...
from raportointi.constants import SKIPPED_TEXT
from raportointi.migrations.testing.setup import (
    ...
    add_testing_responses_kayttooikeus_impersonate_paakayttaja_ok, add_test_localisations,
)
...
class SummaryPostTests(TestCase):
    ...
    def test_post_summary_ok(self):
        ...
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely4")
        post_data = {
        ...
        self.assertEqual(summary.is_skipped, False)
    ...
    @responses.activate
    def test_post_summary_kysely_not_ended_fail(self):
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely2")
        post_data = {"kyselyid": kysely.kyselyid}
        resp = self.client.post(f"{RAPORTOINTIPALVELU_API_URL}/summary/", data=post_data, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue("ER029" in str(resp.json()))

    @responses.activate
    def test_post_summary_laatutekija_fail(self):
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely9")
        post_data = {
            "kyselyid": kysely.kyselyid,
            "group_info": "test group",
            "kuvaus": "testi1",
        ...
        resp = self.client.post(f"{RAPORTOINTIPALVELU_API_URL}/summary/", data=post_data, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue("ER030" in str(resp.json()))

    @responses.activate
    def test_set_summary_skipped_summary_found_ok(self):
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely4")
        post_data = {
            "kyselyid": kysely.kyselyid,
        ...
        # set skipped
        resp = self.client.post(f"{RAPORTOINTIPALVELU_API_URL}/summary/set-skipped/kyselyid={kysely.kyselyid}/")
        ...
        self.assertEqual(summaries.count(), 1)
        summary = summaries.first()
        self.assertEqual(summary.is_skipped, True)
        self.assertEqual(summary.is_locked, True)

    @responses.activate
    def test_set_summary_skipped_summary_not_found_ok(self):
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely4")
        ...
        self.assertEqual(summaries.count(), 1)
        summary = summaries.first()
        self.assertEqual(summary.oppilaitos, kysely.oppilaitos.oid)
        self.assertEqual(summary.group_info, f"{SKIPPED_TEXT} ({kysely.oppilaitos.nimi_fi})")
        self.assertEqual(summary.kuvaus, SKIPPED_TEXT)
        self.assertEqual(summary.is_locked, True)
        self.assertEqual(summary.is_skipped, True)

    @responses.activate
    def test_set_summary_skipped_no_permission_fail(self):
...
class SummaryGetByKyselyIdTests(TestCase):
    ...
    @responses.activate
    def test_get_summary_kysely_not_ended_fail(self):
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely2")
        resp = self.client.get(f"{RAPORTOINTIPALVELU_API_URL}/summary/kyselyid={kysely.kyselyid}/")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue("ER029" in str(resp.json()))
...
class SummaryPdfHtmlCreationTests(TestCase):
    ...
    def test_summary_pdf_html_creation_by_kyselykertaid_validity(self):
        ...
        html_str = create_summary_html(pdf_content, "sv")
        html5validate.validate(html_str)
        self.assertTrue("SV indikaattori" in html_str)
...
class SummaryCsvExportTests(TestCase):
    ...
    @responses.activate
    def test_get_summaries_yllapitaja_csv_ok(self):
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely1")
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/summary/csv/kysymysryhmaid={kysely.kysymysryhmat.first().kysymysryhmaid}/"
            f"pvm1={kysely.voimassa_loppupvm}/pvm2=2040-01-01/"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    @responses.activate
    def test_get_summaries_yllapitaja_csv_base64_ok(self):
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely1")
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/summary/csv/kysymysryhmaid={kysely.kysymysryhmat.first().kysymysryhmaid}/"
            f"pvm1={kysely.voimassa_loppupvm}/pvm2=2040-01-01/?base64=true/"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(type(resp.content), bytes)
        self.assertTrue(resp.content.startswith(b"\xef\xbb"))
    ...
    @responses.activate
    def test_get_summaries_yllapitaja_csv_no_permission(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        kysely = Kysely.objects.get(nimi_fi="summary_testikysely1")
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/summary/csv/kysymysryhmaid={kysely.kysymysryhmat.first().kysymysryhmaid}/"
