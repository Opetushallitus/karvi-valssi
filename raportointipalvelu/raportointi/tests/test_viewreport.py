...
from raportointi.constants import OPETUSHALLITUS_OID, PDF_ANSWERS_HIDDEN_TEXT
...
class ViewReportToteuttajaGetTests(TestCase):
    ...
    def test_get_viewreport_data_toteuttaja(self):
        ...
        self.assertEqual(data["nimi_sv"], "reportview_kysymysryhma_sv")
        self.assertEqual(data["nimi_en"], "reportview_kysymysryhma_en")
        reporting_base = data["reporting_base"]
        self.assertEqual(reporting_base["nimi_fi"], "reportview_kysymysryhma")
        self.assertEqual(reporting_base["nimi_sv"], "reportview_kysymysryhma_sv")
        self.assertEqual(reporting_base["nimi_en"], "reportview_kysymysryhma_en")
    ...
    def test_get_viewreport_kyselykerta_query_data_toteuttaja(self):
        ...
        resp = self.client.get(
            ...
            f"&language_code=fi")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        question_answers = resp.json()["reporting_base"]["questions"][0]["question_answers"]
        self.assertEqual(question_answers["answers_pct"][0][0], 0)
        self.assertEqual(resp.json()["survey_participants_count"], 6)
        self.assertEqual(resp.json()["survey_sent_count"], 6)
        ...
        question_answers = resp.json()["reporting_base"]["questions"][0]["question_answers"]
        self.assertEqual(question_answers["answers_pct"][0][0], 0)
        self.assertEqual(resp.json()["survey_participants_count"], 0)

    @responses.activate
    def test_get_viewreport_hidden_question_toteuttaja_ok(self):
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhma = Kysymysryhma.objects.get(nimi_fi="reportview_kysymysryhma_hq")
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/view-kysymysryhma-report/"
            f"kysymysryhmaid={kysymysryhma.pk}/koulutustoimija=0.1.2/?role=toteuttaja")

        questions = resp.json()["reporting_base"]["questions"]
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(questions), 2)
        self.assertEqual(questions[0]["kysymys_fi"], "view report matrix title hq1")
        self.assertEqual(questions[0]["question_answers"], None)
        self.assertDictEqual(questions[0]["string_answer"], PDF_ANSWERS_HIDDEN_TEXT)
        self.assertEqual(questions[1]["kysymys_fi"], "view report matrix title hq2")
        self.assertEqual(questions[1]["string_answer"], None)
...
class ViewReportPaakayttajaGetTests(TestCase):
    ...
    def test_get_viewreport_query_data_paakayttaja(self):
        ...
        self.assertEqual(data["survey_participants_count"], 0)
        self.assertEqual(data["survey_sent_count"], 0)
        self.assertEqual(data["survey_oppilaitoses_answered_count"], 1)
        self.assertEqual(data["survey_oppilaitoses_activated_count"], 1)
    ...
    @responses.activate
    def test_get_viewreport_hidden_question_paakayttaja_ok(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhma = Kysymysryhma.objects.get(nimi_fi="reportview_kysymysryhma_hq")
        resp = self.client.get(
...
class ViewReportYllapitajaGetTests(TestCase):
    ...
    @responses.activate
    def test_get_viewreport_hidden_question_yllapitaja_ok(self):
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhma = Kysymysryhma.objects.get(nimi_fi="reportview_kysymysryhma_hq")
        resp = self.client.get(
...
class DownloadReportTest(TestCase):
    ...
    @responses.activate
    def test_download_report_pdf_language_sv_ok(self):
        """Check for OK response"""
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        ...
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/download-kysymysryhma-report/"
            f"kysymysryhmaid={kysymysryhma.pk}/koulutustoimija=0.1.2/?language=sv&lomake_language=sv")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.headers["content-type"], "application/pdf")

    @responses.activate
    def test_download_report_pdf_language_en_ok(self):
        """Check for OK response"""
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        ...
        resp = self.client.get(
            f"{RAPORTOINTIPALVELU_API_URL}/download-kysymysryhma-report/"
            f"kysymysryhmaid={kysymysryhma.pk}/koulutustoimija=0.1.2/?lomake_language=en")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.headers["content-type"], "application/pdf")
