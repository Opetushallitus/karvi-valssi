...
class DataCollectionToteuttajaGetTests(TestCase):
    ...
    @responses.activate
    def test_get_datacollection_max_360d_old(self):
        """Check that max 360 days old forms are included"""
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        resp = self.client.get(RAPORTOINTIPALVELU_API_URL + "/data-collection/?role=toteuttaja")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        ...
        for kysely in resp.json():
            self.assertTrue(kysely["voimassa_loppupvm"] >= timenow_minus_360d.strftime(DATE_INPUT_FORMAT))

        # Get Kysely's from database
        kysely_360d_old = Kysely.objects.get(nimi_fi="testikysely_360d_old")
        kysely_361d_old = Kysely.objects.get(nimi_fi="testikysely_361d_old")
        kysely_359d_old = Kysely.objects.get(nimi_fi="testikysely_359d_old")

        # 360d old are shown
        if not any(kysely["nimi_fi"] == kysely_360d_old.nimi_fi for kysely in resp.json()):
            self.assertTrue(False)

        # 361d old are not shown
        if any(kysely["nimi_fi"] == kysely_361d_old.nimi_fi for kysely in resp.json()):
            self.assertTrue(False)

        # 359d old are shown
        if not (kysely["nimi_fi"] == kysely_359d_old.nimi_fi for kysely in resp.json()):
            self.assertTrue(False)
...
class DataCollectionYllapitajaGetTests(TestCase):
    ...
    def test_get_datacollection_statistics(self):
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        timenow_minus_375d = datenow_delta(-375).strftime(DATE_INPUT_FORMAT)
        resp = self.client.get(f"{RAPORTOINTIPALVELU_API_URL}/data-collection/"
                               f"?role=yllapitaja&start_date={timenow_minus_375d}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

        timenow = timezone.now().strftime(DATE_INPUT_FORMAT)
        timenow_plus_30d = datenow_delta(30).strftime(DATE_INPUT_FORMAT)
        timenow_minus_370d = datenow_delta(-370).strftime(DATE_INPUT_FORMAT)
        ...
        self.assertEqual(stats["kunnallinen"]["earliest_usage_date"], timenow)
        self.assertEqual(stats["kunnallinen"]["latest_ending_date"], timenow_plus_30d)
        koulutustoimija_names = (
            stats["kunnallinen"]["koulutustoimija_statistics"]["extra_data"]["koulutustoimija_names"]["fi"])
        self.assertEqual(len(koulutustoimija_names), 1)
        self.assertEqual(koulutustoimija_names[0], "datacollection_toimija")
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["oppilaitos_answered_count"], 0)
        ...
        self.assertEqual(stats["kunnallinen"]["koulutustoimija_statistics"]["in_use_count"], 2)
        self.assertEqual(stats["kunnallinen"]["koulutustoimija_statistics"]["sent_count"], 2)
        self.assertEqual(stats["kunnallinen"]["oppilaitos_statistics"]["in_use_count"], 6)
        self.assertEqual(stats["kunnallinen"]["oppilaitos_statistics"]["sent_count"], 6)
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["sent_count"], 17)
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["answered_count"], 7)
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["answer_pct"], 41)
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["oppilaitos_answered_count"], 5)

        stats = resp.json()[2]
        self.assertEqual(stats["nimi_fi"], "testikysymysryhma1")
        self.assertEqual(stats["kunnallinen"]["earliest_usage_date"], timenow_minus_370d)
        koulutustoimija_names = (
            stats["kunnallinen"]["koulutustoimija_statistics"]["extra_data"]["koulutustoimija_names"]["fi"])
        self.assertEqual(len(koulutustoimija_names), 1)
        self.assertEqual(koulutustoimija_names[0], "testikoulutustoimija1")
        self.assertEqual(stats["kunnallinen"]["vastaaja_statistics"]["oppilaitos_answered_count"], 1)
    ...
    def test_get_datacollection_end_date(self):
        ...
        timenow_minus_30d = datenow_delta(-30).strftime(DATE_INPUT_FORMAT)
        timenow_minus_375d = datenow_delta(-375).strftime(DATE_INPUT_FORMAT)
        resp = self.client.get(f"{RAPORTOINTIPALVELU_API_URL}/data-collection/?role=yllapitaja"
                               f"&start_date={timenow_minus_375d}&end_date={timenow_minus_30d}")
