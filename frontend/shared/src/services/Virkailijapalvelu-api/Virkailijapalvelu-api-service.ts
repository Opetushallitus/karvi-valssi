...
import {MatrixType, SekondaarinenIndikaattoriType, TextType} from '../Data/Data-service';
...
export type OppilaitosType = {
    name: TextType;
    oid: string;
};

export type OppilaitosGroupType = {
    id: string | null;
    name: TextType;
    oppilaitokset: OppilaitosType[];
};

export type OppilaitosSetType = {
    grouped: OppilaitosGroupType[];
    ungrouped: OppilaitosGroupType[];
};
...
export type IndikaattoriGroupType = {
    group_id: number;
    indicators: SekondaarinenIndikaattoriType[];
};
...
export type VirkailijapalveluUpdateKyselyResponseType = {
    updates: number;
};
...
export type AlueFormType = {
    koulutustoimija: string;
    name_fi: string;
    name_sv: string;
    toimipaikkaList: OppilaitosSetType;
};
...
const aluejakoApi = `${virkailijapalveluApiBase}aluejako/`;
...
export const virkailijapalveluSendUpdateKysely$ =
    (user: UserInfoType) => (body: VirkailijapalveluKyselySendUpdateObject) =>
        virkailijapalveluHttpImpersonation(
            user,
        ).put<VirkailijapalveluUpdateKyselyResponseType>(`${kyselysendApi}update/`, body);

export const virkailijapalveluSendReminderKysely$ =
    (user: UserInfoType) => (kyselykertaId: number) =>
        virkailijapalveluHttpImpersonation(
            user,
        ).put<VirkailijapalveluUpdateKyselyResponseType>(
            `${kyselysendApi}reminder/?kyselykerta=${kyselykertaId}`,
        );

/**
 * Check CAS logged in user rights for Valssi
 * @param user
 */
export const virkailijapalveluGetPermission$ = (user: UserInfoType) =>
    virkailijapalveluHttpImpersonation(user).get<string>(
        virkailijapalveluApiBase + 'getpermission/',
    );
...
export const virkailijapalveluGetPdfKysymysryhma$ =
    ...
    (kysymysryhmaId: number, language = 'fi') =>
        virkailijapalveluHttpImpersonation(user).get<string>(
            `${kysymysryhmaApi}${kysymysryhmaId}/pdf/`,
            {language, base64: true},
        );

/**
 * Set kysymysryhma status as archived
 * @param kysymysryhmaId
 */
...
/**
 * Set kysymysryhma status as locked
 * @param kysymysryhmaId
 */
export const virkailijapalveluSetKysymysryhmaLocked$ =
    (user: UserInfoType) => (kysymysryhmaId: number) =>
        virkailijapalveluHttpImpersonation(user).patch<UpdatedNumberType>(
            `${kysymysryhmaApi}${kysymysryhmaId}/set-locked/`,
        );

/**
 * Set kysymysryhma status from locked to published
 * @param kysymysryhmaId
 */
export const virkailijapalveluSetKysymysryhmaUnlocked$ =
    (user: UserInfoType) => (kysymysryhmaId: number) =>
        virkailijapalveluHttpImpersonation(user).patch<UpdatedNumberType>(
            `${kysymysryhmaApi}${kysymysryhmaId}/set-unlocked/`,
        );
...
export const virkailijapalveluGetProsessitekijaIndikaattorit$ =
    (user: UserInfoType) => () =>
        virkailijapalveluHttpImpersonation(user).get<IndikaattoriGroupType[]>(
            `${indikaattoriApi}`,
...
/**
 * get all kyselylomake and kansalliset indicators
 */
export const virkailijapalveluGetKansallisetIndikaattorit$ = (user: UserInfoType) => () =>
    virkailijapalveluHttpImpersonation(user).get<IndikaattoriGroupType[]>(
        `${indikaattoriApi}`,
        {laatutekija: 'kansallinen'},
    );
...
export const virkailijapalveluGetIndikaattoriRyhma$ =
    (user: UserInfoType) => (group_id?: number) =>
        virkailijapalveluHttpImpersonation(user).get<IndikaattoriGroupType[]>(
            `${indikaattoriApi}`,
            {group_id},
        );
...
export const virkailijapalveluGetTyontekijaList$ =
    (user: UserInfoType) => (toimipaikka: string) =>
        virkailijapalveluHttpImpersonation(user).get<ToimipaikanTyontekijaType[]>(
            `${tyontekijaApi}list/toimipaikka=${toimipaikka}/`,
        );

export const virkailijapalveluPostSendKyselyList$ =
    (user: UserInfoType) => (id: number) =>
        virkailijapalveluHttpImpersonation(user).post<HasBeenSentToPeopleType[]>(
...
/* get aluejako per koulutustoimija */
export const virkailijapalveluGetAluejako$ =
    (user: UserInfoType) => (koulutustoimijaOid: string) =>
        virkailijapalveluHttpImpersonation(user).get<OppilaitosSetType>(
            `${aluejakoApi}list/koulutustoimija=${koulutustoimijaOid}/`,
        );

export const virkailijapalveluCreateAlue$ = (user: UserInfoType) => (body: any) =>
    virkailijapalveluHttpImpersonation(user).post<any>(`${aluejakoApi}create/`, body);

export const virkailijapalveluModifyAlue$ = (user: UserInfoType) => (body: any) =>
    virkailijapalveluHttpImpersonation(user).put<any>(`${aluejakoApi}modify/`, body);

export const virkailijapalveluDeleteAlue =
    (user: UserInfoType) => (body: {koulutustoimija: string; id: number}) =>
        virkailijapalveluHttpImpersonation(user).delete<any>(
            `${aluejakoApi}delete/`,
            body,
        );
