...
import {
    ...
    convertJatkokysymyksetArvoToValssi,
...
function LisaaMuokkaaKysymys({
    ...
    const [isHiddenOnReportFlag, setIsHiddenOnReportFlag] = useState(false);

    const [followUpData, setFollowUpData] = useState<FollowupQuestionsType>({});

    const [matrixScales] = useObservableState(() => virkailijapalveluGetMatrixScales$());
    ...
    const setDefaultValues = (kysymys: KysymysType, qType: QuestionTypes) => {
        ...
        setDescription({
            ...
            en: kysymys.description?.en ?? '',
        });
        setFollowUpData(kysymys?.followupQuestions || {});
    };

    const openDialog = () => {
        setselectedQuestionType(QuestionTypes.matrix);
        setSelectedInputType(InputTypes.matrix_radio);
        setTitle(showEnglish ? {fi: '', sv: '', en: ''} : {fi: '', sv: ''});
        setDescription(showEnglish ? {fi: '', sv: '', en: ''} : {fi: '', sv: ''});
        setAnswers([]);
        setSubQuestions([]);
        setOrigSubQuestions([]);
        ...
        if (selectedKysymysId !== -1) {
            ...
            if (kysymys) {
                ...
                setPage(kysymys.page ? kysymys.page : 0);
                setPagebreak(kysymys.pagebreak ? kysymys.pagebreak : false);
                setIsHiddenOnReportFlag(
                    kysymys.metatiedot?.is_hidden_on_report
                        ? kysymys.metatiedot?.is_hidden_on_report
                        : false,
                );
                switch (kysymys.inputType) {
                    ...
                    case InputTypes.matrix_radio:
                        ...
                        break;
                    default:
                        console.warn(`Error: Invalid input-type: ${kysymys.inputType}`);
                        onDialogClose();
                        break;
                }
            } else {
                console.warn(`Error: Kysymys not found! Id: ${selectedKysymysId}`);
            }
        }
        setShowDialog(true);
    };
    ...
    const handleChange = (changeFromSubComponent: KysymysDataType) => {
        ...
        } else if (changeFromSubComponent.followUpData) {
            setFollowUpData(changeFromSubComponent.followUpData);
        }
    };
    ...
    const handleQuestionTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (newValue === QuestionTypes.text) {
            ...
            setSelectedInputType(InputTypes.radio);
    ...
    const handleIsHiddenOnReport = (isHiddenOnReport: boolean) => {
        setIsHiddenOnReportFlag(isHiddenOnReport);
        setSubQuestions(
            subQuestions.map((sq) => ({
                ...sq,
                metatiedot: {...sq.metatiedot, is_hidden_on_report: isHiddenOnReport},
            })),
        );
    };
    ...
    const handleSave = () => {
        const updatedKysymys: KysymysType = {
            ...
            title: showEnglish
                ? {fi: title.fi, sv: title.sv, en: title.en}
                : {fi: title.fi, sv: title.sv},
            description: showEnglish
                ? {fi: description.fi, sv: description.sv, en: description.en}
                : {fi: description.fi, sv: description.sv},
            ...
            page,
            pagebreak,
            metatiedot: existingKysymys
                ? {
                      ...existingKysymys.metatiedot,
                      is_hidden_on_report: isHiddenOnReportFlag,
                  }
                : {is_hidden_on_report: isHiddenOnReportFlag},
        };
        ...
        saveKysymysDb(kysely.id, updatedKysymys).subscribe((res) => {
            ...
            if (res?.jatkokysymykset && Object.keys(res?.jatkokysymykset).length > 0) {
                updatedKysymys.followupQuestions = convertJatkokysymyksetArvoToValssi(
                    res.jatkokysymykset,
                    0, // dummy value
                );
            }
            ...
            case InputTypes.dropdown:
                return (
                    <CheckboxQuestion
                        ...
                        kysymykset={kysely.kysymykset.filter(
                            (kys) => kys.id !== selectedKysymysId,
                        )}
            ...
            {showDialog && (
                ...
                >
                    ...
                    <div className={styles['form-controls__container']}>
                        ...
                        <div>
                            <FormControlLabel
                                label={t(`kysymysta-ei-nayteta-raportilla-label`)}
                                control={
                                    <Checkbox
                                        key="isHiddenOnReportField"
                                        checked={isHiddenOnReportFlag}
                                        onChange={() =>
                                            handleIsHiddenOnReport(!isHiddenOnReportFlag)
                                        }
                                        name="isHiddenOnReportField"
                                        className={styles.checkbox}
                                        disabled={false}
                                    />
                                }
