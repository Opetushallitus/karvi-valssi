...
from kyselyt.constants import (
    ...
    EMAIL_STATUS_FAILED, EMAIL_STATUS_DELIVERED, TILAENUM_PUBLISHED, OPETUSHALLITUS_OID, ALLOWED_TOIMINTAMUOTO_CODES,
    ANONYMIZED_TOIMINTAMUOTO_CODES,
)
...
from kyselyt.models import (
    ...
    Organisaatio, Vastaajatunnus, Scale, Koodi, ExternalServices, MalfunctionMessage, Localisation,
)
...
def add_test_kyselyt_kyselykerrat():
    ...
    voimassa_kwargs = dict(
        ...
        voimassa_loppupvm=time_now + timedelta(days=30))
    tila_julkaistu, _ = TilaEnum.objects.get_or_create(nimi=TILAENUM_PUBLISHED)
    kysymysryhma_kwargs = dict(taustakysymykset=False, valtakunnallinen=False)
    kysely2 = Kysely.objects.create(
        nimi_fi="testikysely2", koulutustoimija=vakajarjestaja, **kayttajat_kwargs, **voimassa_kwargs)
    kysymysryhma1 = Kysymysryhma.objects.create(
        nimi_fi="testikysymysryhma1", tila=tila_julkaistu, **kayttajat_kwargs, **kysymysryhma_kwargs)
    Kysymysryhma.objects.create(
        nimi_fi="testikysymysryhma2", tila=tila_julkaistu, **kayttajat_kwargs, **kysymysryhma_kwargs)
    ...
    kysymysryhma9_1 = Kysymysryhma.objects.create(
        nimi_fi="testikysymysryhma9_1", nimi_sv="testikysymysryhma9_1_sv", nimi_en="testikysymysryhma9_1_en",
...
def add_test_kyselysends():
    ...
    KyselySend.objects.create(
        email="a1@ksu.aa", kyselykerta=kyselykerta3.kyselykertaid, vastaajatunnus="kyse1", msg_id=111,
        msg_status=EMAIL_STATUS_FAILED)
    KyselySend.objects.create(
        email="a2@ksu.aa", kyselykerta=kyselykerta3.kyselykertaid, vastaajatunnus="kyse2", msg_id=112,
        msg_status=EMAIL_STATUS_DELIVERED)
    KyselySend.objects.create(
        email="a3@ksu.aa", kyselykerta=999, vastaajatunnus="kyse3", msg_id=113, msg_status=EMAIL_STATUS_FAILED)


def add_test_kyselysend_vastaajatunnuses():
    # for kyselysend reminder tests
    kayttaja = Kayttaja.objects.get(oid="testikayttaja1_oid")
    kayttajat_kwargs = dict(luotu_kayttaja=kayttaja, muutettu_kayttaja=kayttaja)
    time_now = timezone.now()
    voimassa_kwargs = dict(
        voimassa_alkupvm=time_now - timedelta(days=30),
    ...
    Vastaajatunnus.objects.create(
        kyselykertaid=kyselykerta3, tunnus="kyse1", kohteiden_lkm=1, lukittu=False,
        **kayttajat_kwargs, **voimassa_kwargs)
    Vastaajatunnus.objects.create(
        kyselykertaid=kyselykerta3, tunnus="kyse2", kohteiden_lkm=1, lukittu=False,
        **kayttajat_kwargs, **voimassa_kwargs)


def add_test_indikaattorit():
    Indikaattori.objects.create(group_id=11, indicator_key="a1", laatutekija="a", is_visible=True)
    Indikaattori.objects.create(group_id=11, indicator_key="a2", laatutekija="a", is_visible=True)
    Indikaattori.objects.create(group_id=12, indicator_key="a3", laatutekija="a", is_visible=True)
    Indikaattori.objects.create(group_id=21, indicator_key="b1", laatutekija="b", is_visible=True)
    Indikaattori.objects.create(group_id=21, indicator_key="b2", laatutekija="b", is_visible=True)
    Indikaattori.objects.create(group_id=21, indicator_key="b3", laatutekija="b", is_visible=False)
...
def add_testing_responses_kayttooikeus_yllapitaja_ok(responses_obj):
    responses_obj.add(
        ...
        json=[{
            "oidHenkilo": "test-oidhenkilo",
            "organisaatiot": [
                {"organisaatioOid": OPETUSHALLITUS_OID,
...
def add_testing_responses_kayttooikeus_toteuttaja_getpermission_allowed_toimipaikka(responses_obj):
    responses_obj.add(
        responses_obj.GET, f"{settings.KAYTTOOIKEUS_SERVICE_URL}kayttooikeus/kayttaja?username=test-user",
        json=[
            {
...
def add_testing_responses_kayttooikeus_toteuttaja_getpermission_anonymized_toimipaikka(responses_obj):
    responses_obj.add(
        responses_obj.GET, f"{settings.KAYTTOOIKEUS_SERVICE_URL}kayttooikeus/kayttaja?username=test-user",
        json=[
            {
...
def add_testing_responses_kayttooikeus_toteuttaja_getpermission_tm_code_missing(responses_obj):
    responses_obj.add(
        responses_obj.GET, f"{settings.KAYTTOOIKEUS_SERVICE_URL}kayttooikeus/kayttaja?username=test-user",
        json=[
            {
...
def add_testing_responses_organisaatiot_update(responses_obj):
    ...
    responses_obj.add(
        responses_obj.POST,
        settings.ARVO_ORGANISAATIOT_UPDATE_ENDPOINT,
...
def add_testing_responses_toimipaikat_update(responses_obj):
            ...
            "results": [
                {
                    ...
                    "action": "CREATED",
                    "nimi": "päiväkoti",
                    "organisaatio_oid": "1.2.246.562.10.346830761115",
                    "parent_oid": "1.2.246.562.10.346830761110",
                    "organisaatiotyyppi": [],
                    "postinumero": "12345",
    ...
    responses_obj.add(
        responses_obj.POST,
        settings.VARDA_TOIMIPAIKAT_ENDPOINT,
        status=status.HTTP_201_CREATED
    )
...
def add_test_localisations():
    ...
    Localisation.objects.create(
        key="testkey1", locale="fi", value="fi_value", **localisation_kwargs)
    Localisation.objects.create(
        key="testkey1", locale="sv", value="sv_value", **localisation_kwargs)
    Localisation.objects.create(
        key="testkey2", locale="fi", value="fi_value", **localisation_kwargs)
...
def add_testing_responses_taustatiedot_missing(responses_obj):
    ...
    responses_obj.add(
        responses_obj.GET, addr, json={
            ...
            "tyontekijat": {},
            "taydennyskoulutukset": {},
            "tuen_tiedot": {}}, status=status.HTTP_200_OK)
...
def add_test_aluejako_organisaatiot():
    vakajarjestaja = Organisaatio.objects.create(
        nimi_fi="testivakajarjestaja1", oid="0.1.2", kunta="001", voimassa=True,
        metatiedot=dict())
    ...
    for i in range(1, 5):
        Organisaatio.objects.create(
            nimi_fi=f"testitoimipaikka{i}", oid=f"0.1.2.{i}", postinumero="12345", voimassa=True,
            parent_oid=vakajarjestaja,
            metatiedot=dict(jarjestamismuoto_koodit=["jm01"], toimintamuoto_koodi="tm01"))


def add_test_varda_update_organisaatiot_aluejako():
    vakajarjestaja = Organisaatio.objects.create(
        nimi_fi="testivakajarjestaja1",
        oid="0.1.2",
        kunta="001",
        voimassa=True,
        metatiedot=dict()
    ...
    Organisaatio.objects.create(
        nimi_fi="testitoimipaikka1",
        oid="0.1.2.1",
        parent_oid=vakajarjestaja,
        postinumero="12345",
        voimassa=True,
        metatiedot=dict(
            jarjestamismuoto_koodit=["jm01"],
...
def get_test_varda_organisaatiot_data():
    return [
        {
            "id": 10,
            "action": "CREATED",
            "action_timestamp": "2025-11-01T12:00:00.123456Z",
            "nimi": "testivakajarjestaja1",
            "y_tunnus": "1231234-1",
            "organisaatio_oid": "0.1.2",
            "parent_oid": OPETUSHALLITUS_OID,
            "organisaatiotyyppi": [
            ...
            "kunta_koodi": "001",
            "sahkopostiosoite": "email@example.com",
            "kayntiosoite": "PL",
            "kayntiosoite_postinumero": "12345",
            "kayntiosoite_postitoimipaikka": "Hki",
            "postiosoite": "PL",
            "postinumero": "12345",
            "postitoimipaikka": "Hki",
            "puhelinnumero": "+358123",
...
def get_test_varda_toimipaikat_data():
    response = []
    for i in [1, 2, 3]:
        response.append(
            {
                ...
                "action": "MODIFIED",
                "action_timestamp": "2025-11-01T12:00:00.123456Z",
                "nimi": f"testitoimipaikka{i}",
                "organisaatio_oid": f"0.1.2.{i}",
                "parent_oid": "0.1.2",
                "organisaatiotyyppi": ["organisaatiotyyppi_08"],
                "postinumero": "12345",
                "jarjestamismuoto_koodi": ["jm01"],
                "toimintamuoto_koodi": f"tm0{i}",
                "toimintakieli_koodi": ["fi"],
                "alkamis_pvm": "2000-01-01",
                "paattymis_pvm": "2030-12-31"
            }
        )
    return response


def get_test_varda_organisaatiot_data_expected_data():
    return {
        "oid": "0.1.2",
        "parentOid": "1.2.246.562.10.00000000001",
        "nimi": {
            "fi": "testivakajarjestaja1",
            "sv": "testivakajarjestaja1",
            "en": "testivakajarjestaja1"
        },
        "yhteystiedot": [],
        "tyypit": [
            "organisaatiotyyppi_01",
...
def get_test_varda_toimipaikat_data_expected_data():
    response = []
    for i in [1, 2, 3]:
        oid = f"0.1.2.{i}"
        toimintamuoto_koodi = f"tm0{i}"

        if toimintamuoto_koodi in ALLOWED_TOIMINTAMUOTO_CODES:
            nimi = f"testitoimipaikka{i}"
        elif toimintamuoto_koodi in ANONYMIZED_TOIMINTAMUOTO_CODES:
            nimi = f"anonymized-{oid[-8:]}"
        else:
            continue
        ...
        response.append(
            {
                ...
                "metatiedot": {
                    "jarjestamismuoto_koodit": ["jm01"],
                    "toimintamuoto_koodi": toimintamuoto_koodi,
                    "toimintakieli_koodi": ["FI"]
                },
                "ytunnus": "0000000-0",
                "lakkautusPvm": "2030-12-31",
                "postiosoite": {
                    "postinumeroUri": "posti_12345",
                    "osoite": "",
                    "postitoimipaikka": ""
...
def add_test_organisaatiot_getpermission():
    oph = Organisaatio.objects.create(
        nimi_fi="oph",
        oid=OPETUSHALLITUS_OID,
        kunta="001",
        voimassa=True,
        metatiedot=dict(),
    )
    vakajarjestaja = Organisaatio.objects.create(
        nimi_fi="testivakajarjestaja",
        oid="0.1.2",
        parent_oid=oph,
        kunta="001",
        voimassa=True,
    ...
    Organisaatio.objects.create(
        nimi_fi="testitoimipaikka_tm02",
        oid="0.1.2.2",
        parent_oid=vakajarjestaja,
        postinumero="12345",
        voimassa=True,
        metatiedot=dict(jarjestamismuoto_koodit=["jm01"], toimintamuoto_koodi="tm02"),
    )
    Organisaatio.objects.create(
        nimi_fi="testitoimipaikka_tm_missing",
        oid="0.1.2.3",
        parent_oid=vakajarjestaja,
        postinumero="12345",
        voimassa=True,
        metatiedot=dict(jarjestamismuoto_koodit=["jm01"]),
    )
