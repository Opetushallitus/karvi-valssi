...
class KyselykertaExtendEnddateApiTests(TestCase):
    ...
    @responses.activate
    def test_patch_kyselykerta_extend_enddate_enddate_current_date_ok(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhmaid = Kysymysryhma.objects.get(nimi_fi="testikysymysryhma10").pk
        kysely = Kysely.objects.get(nimi_fi="testikysely10")
        alkupvm = datetime.strftime(kysely.voimassa_alkupvm, DATE_INPUT_FORMAT)
        new_voimassa_loppupvm = datetime.now().date()
        data = {"voimassa_loppupvm": datetime.strftime(new_voimassa_loppupvm, DATE_INPUT_FORMAT)}
        resp = self.client.patch(
            f"{VIRKAILIJAPALVELU_API_URL}/kyselykerta/extend-enddate/kysymysryhmaid={kysymysryhmaid}/"
            f"koulutustoimija={kysely.koulutustoimija.oid}/alkupvm={alkupvm}/",
    ...
    @responses.activate
    def test_patch_kyselykerta_extend_enddate_kysely_enddate_in_the_past(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
    ...
    @responses.activate
    def test_patch_kyselykerta_extend_enddate_enddate_same_as_startdate_fail(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhmaid = Kysymysryhma.objects.get(nimi_fi="testikysymysryhma10").pk
        kysely = Kysely.objects.get(nimi_fi="testikysely10")
        alkupvm = datetime.strftime(kysely.voimassa_alkupvm, DATE_INPUT_FORMAT)
        data = {"voimassa_loppupvm": alkupvm}
        resp = self.client.patch(
            f"{VIRKAILIJAPALVELU_API_URL}/kyselykerta/extend-enddate/kysymysryhmaid={kysymysryhmaid}/"
            f"koulutustoimija={kysely.koulutustoimija.oid}/alkupvm={alkupvm}/",
            data, format="json")
    ...
    @responses.activate
    def test_patch_kyselykerta_extend_enddate_enddate_before_current_date_fail(self):
        add_testing_responses_kayttooikeus_paakayttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        kysymysryhmaid = Kysymysryhma.objects.get(nimi_fi="testikysymysryhma10").pk
        kysely = Kysely.objects.get(nimi_fi="testikysely10")
        alkupvm = datetime.strftime(kysely.voimassa_alkupvm, DATE_INPUT_FORMAT)
        new_voimassa_loppupvm = datetime.now().date() - timedelta(days=1)
        data = {"voimassa_loppupvm": datetime.strftime(new_voimassa_loppupvm, DATE_INPUT_FORMAT)}
        resp = self.client.patch(
            f"{VIRKAILIJAPALVELU_API_URL}/kyselykerta/extend-enddate/kysymysryhmaid={kysymysryhmaid}/"
            f"koulutustoimija={kysely.koulutustoimija.oid}/alkupvm={alkupvm}/",
            data, format="json")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertTrue("VA010" in str(resp.json()))
