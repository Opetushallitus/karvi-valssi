...
from kyselyt.migrations.testing.setup import (
    ...
    add_testing_responses_toimipaikat_update, add_outdated_kyselysend, add_test_kyselykerta_for_html_test,
    add_testing_responses_kayttooikeus_userauthorization, add_test_localisations,
    add_test_varda_update_organisaatiot_aluejako, get_test_varda_organisaatiot_data, get_test_varda_toimipaikat_data,
    get_test_varda_organisaatiot_data_expected_data, get_test_varda_toimipaikat_data_expected_data,
)
from kyselyt.models import (
    ExternalServices, Kyselykerta, KyselySend, Scale, Kysymysryhma, UserAuthorization,
)
from kyselyt.utils import (
    ...
    create_arvo_organizations_update_data,
)
...
class VardaUpdateTests(TestCase):
    ...
    def test_create_arvo_organizations_update_data_organisaatiot(self):
        orgs_data = get_test_varda_organisaatiot_data()
        data = create_arvo_organizations_update_data(orgs_data)
        expected_data = get_test_varda_organisaatiot_data_expected_data()
        self.assertDictEqual(data[0], expected_data)

    def test_create_arvo_organizations_update_data_toimipaikat(self):
        add_test_varda_update_organisaatiot_aluejako()

        orgs_data = get_test_varda_toimipaikat_data()
        data = create_arvo_organizations_update_data(orgs_data)
        expected_data = get_test_varda_toimipaikat_data_expected_data()
        expected_data[0]["metatiedot"]["aluejako"] = 1
        for i in range(len(data)):
            self.assertDictEqual(data[i], expected_data[i])
...
class LocalisationValuesTests(TestCase):
    ...
    def test_get_localisation_values_by_key_ok(self):
        values = get_localisation_values_by_key("testkey1")
    ...
    def test_get_localisation_values_by_key_sv_missing_ok(self):
        values = get_localisation_values_by_key("testkey2")
        self.assertEqual(values["fi"], "fi_value")
        self.assertEqual(values["sv"], "testkey2")

    def test_get_localisation_values_by_key_key_missing_ok(self):
        values = get_localisation_values_by_key("testkey3")
        self.assertEqual(values["fi"], "testkey3")
        self.assertEqual(values["sv"], "testkey3")
