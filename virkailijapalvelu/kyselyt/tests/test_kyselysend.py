...
from kyselyt.migrations.testing.setup import (
    ...
    add_testing_responses_kayttooikeus_impersonate_toteuttaja_ok, add_test_kyselysend_vastaajatunnuses)
...
class KyselysendPostTests(TestCase):
    ...
    def test_post_kyselysend_taydennyskoulutuslomake_rakennetekijat_ok(self):
        ...
        tuen_tiedot = kyselykerta.metatiedot["rakennetekijalomake_data"]["tuen_tiedot"]
        self.assertEqual(tuen_tiedot["tilastointi_pvm"], "2024-05-31")
        self.assertEqual(tuen_tiedot["kunnallinen"]["tuen_tasot"]["TT01"]["ikaryhmat"]["IR01"], 0)
        self.assertEqual(tuen_tiedot["kunnallinen"]["tuen_tasot"]["TT02"]["ikaryhmat"]["IR03"], 6)
        self.assertEqual(tuen_tiedot["kunnallinen"]["tuen_tasot"]["TT03"]["ikaryhmat"]["IR05"], 2)
        self.assertEqual(tuen_tiedot["yksityinen"], {})
    ...
    def test_post_kyselysend_taydennyskoulutuslomake_rakennetekijat_taustatiedot_missing(self):
        ...
        self.assertEqual(kyselykerta.metatiedot["rakennetekijalomake_data"]["tuen_tiedot"], {})
...
@override_settings(RATELIMIT_ENABLE=False)
class KyselysendReminderTests(TestCase):
    databases = ["default", "valssi"]
    ...
    @responses.activate
    def test_put_kyselysend_reminder_ok(self):
        add_testing_responses_viestintapalvelu_token(responses)
        add_testing_responses_viestintapalvelu_laheta_update(responses)
        add_testing_responses_kayttooikeus_ok(responses)
        add_testing_responses_service_ticket(responses)
        add_test_kyselysends()
        add_test_kyselysend_vastaajatunnuses()
        kyselykerta = Kyselykerta.objects.get(nimi="testikyselykerta3")
        resp = self.client.put(f"{VIRKAILIJAPALVELU_API_URL}/kyselysend/reminder/?kyselykerta={kyselykerta.pk}")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.json().get("updates"), 2)

    @responses.activate
    def test_put_kyselysend_reminder_impersonate_ok(self):
        add_testing_responses_viestintapalvelu_token(responses)
        add_testing_responses_viestintapalvelu_laheta_update(responses)
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        add_testing_responses_service_ticket(responses)
        ...
        kyselykerta = Kyselykerta.objects.get(nimi="testikyselykerta3")
        resp = self.client.put(
            f"{VIRKAILIJAPALVELU_API_URL}/kyselysend/reminder/?kyselykerta={kyselykerta.pk}",
            HTTP_IMPERSONATE_USER="test-impersonate-user")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.json().get("updates"), 2)

    @responses.activate
    def test_put_kyselysend_reminder_impersonate_org_ok(self):
        add_testing_responses_viestintapalvelu_token(responses)
        add_testing_responses_viestintapalvelu_laheta_update(responses)
        add_testing_responses_kayttooikeus_yllapitaja_ok(responses)
        add_testing_responses_service_ticket(responses)
    ...
    @responses.activate
    def test_put_kyselysend_reminder_toteuttaja_ok(self):
        add_testing_responses_viestintapalvelu_token(responses)
        add_testing_responses_viestintapalvelu_laheta_update(responses)
        add_testing_responses_kayttooikeus_toteuttaja_ok(responses)
        add_testing_responses_service_ticket(responses)
    ...
    def test_get_kyselysend_reminder_fail(self):
        resp = self.client.get(f"{VIRKAILIJAPALVELU_API_URL}/kyselysend/reminder/")
        self.assertEqual(resp.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_post_kyselysend_reminder_fail(self):
        resp = self.client.post(f"{VIRKAILIJAPALVELU_API_URL}/kyselysend/reminder/")
        self.assertEqual(resp.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
