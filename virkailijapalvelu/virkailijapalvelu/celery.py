...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(INTERVAL * 60.0, update_localisations_task.s())
...
@app.task
def update_localisations_task():
    from kyselyt.utils import create_or_update_localisations

    created_count, update_count = create_or_update_localisations()
    if created_count or update_count:
        logger.info(f"Localisations checked and updated. Created: {created_count}, Updated: {update_count}")
    else:
        logger.info("Localisations checked. No changes.")
