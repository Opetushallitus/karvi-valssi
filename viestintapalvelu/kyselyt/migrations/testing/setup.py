...
def add_testing_responses_email_service(responses_obj):
    responses_obj.add(
        responses_obj.POST, f"{settings.EMAIL_SERVICE_URL}/viestit",
        json={"lahetysTunniste": "tunniste-1"}, status=status.HTTP_200_OK)

    responses_obj.add(
        responses_obj.POST, f"{settings.EMAIL_SERVICE_URL}/liitteet",
        json={"liiteTunniste": "liitetunniste-1"}, status=status.HTTP_200_OK)
